...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/proxmox_v9x/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/proxmox_v9x/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/proxmox_v9x/*"
            )
            ...
            #    "--include=src/saltext/proxmox_v9x/*,tests/*"
            # )
        finally:
